...
export default class SVGPathCollider {
	constructor(path, separationNum = 16, isConcave = false) {
		...
		this.boundingBox = new SAT.Polygon(new SAT.Vector(), this.times(4, () => new SAT.Vector()));
		this.collisionArea = new SAT.Polygon(new SAT.Vector(), this.times(separationNum, () => new SAT.Vector()));
		var ose = path.ownerSVGElement;
		this.boundingPoints = this.times(4, () => ose.createSVGPoint());
		if (isConcave) {
			this.concaveCollisionAreas = this.times(separationNum, () => new SAT.Polygon(new SAT.Vector(), this.times(3, () => new SAT.Vector())));
		}
	}
	...
	update() {
		...
		if (this.isShowingCollision) {
			var visibility = this.isBoundingBoxColliding
				? "visible"
				: "hidden";
	...
	testToPolygon(polygon) {
		if (this.isConcave) {
			return this.concaveCollisionAreas.some(cca => SAT.testPolygonPolygon(cca, polygon));
		} else {
